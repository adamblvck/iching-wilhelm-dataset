...
- `wilhelm_judgment`: The "Law", if the Hexagram would be judging you, or the world, this would be the text that represents it. Contains `text` and `comments` parts. The `text` part contains the traditional poem-like description, the `comments` contain Wilhelm's comments on this text.
